...
- `UNAUTHENTICATED` if the service token or session context is invalid. The public app maps this boundary to `BILLING_UNAUTHORIZED` or `BILLING_FORBIDDEN` when it receives a 401 or 403.
...
- `ARTIFACT_LOCKED` is generated by the public app after a successful access response when `has_access=false` and a protected route needs to block access.
...
- `401` for `UNAUTHENTICATED` or service-token failures that the public app surfaces as `BILLING_UNAUTHORIZED`
